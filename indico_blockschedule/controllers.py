...
def _grid_payload(event, day, *, full_day=False, accessible_only=False, manage=True):
    ...
    if manage:
        ...
        payload['unscheduled_contributions'] = [serialize_contribution(c, description_display)
...
class RHManageGridData(RHBlockScheduleManageBase):
    def _process(self):
        ...
        return _conditional_response(jsonify(_grid_payload(self.event, day, full_day=True)))
...
class RHDisplayGridData(RHDisplayEventBase):
    def _process(self):
        ...
        return _conditional_response(jsonify(_grid_payload(self.event, day, accessible_only=True, manage=False)))
